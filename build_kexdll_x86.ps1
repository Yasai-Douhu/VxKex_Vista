...
$kexPathCchLib = Join-Path $ScriptDirAbs "VxKex-NEXT\Win32\Release\KexPathCch\KexPathCch.lib"
...
$libArgs = @("/NOLOGO", "/DLL", "/INCREMENTAL:NO", "/SUBSYSTEM:WINDOWS", "/OPT:REF", "/OPT:ICF", "/DEBUG", "/RELEASE", "/MACHINE:X86", "/LTCG")
$libArgs += @("/OUT:`"$dllPath`"", "/IMPLIB:`"$libPath`"", "/DEF:`"$defFile`"")
$libArgs += @("/LIBPATH:`"$importLibsDir`"", "/LIBPATH:`"$SDK71_LIB`"")
$libArgs += @("ntdll.lib", "kernel32.lib", "user32.lib", "gdi32.lib", "shlwapi.lib")
$libArgs += @("`"$kexSmpLib`"", "`"$kexPathCchLib`"")
$libArgs += $objFiles
$libArgs += @("`"$resFile`"")

Write-Host "Linking KexDll.dll (x86)..."
& "link.exe" @libArgs
if ($LASTEXITCODE -ne 0) { exit 1 }

Write-Host "KexDll.dll (x86) built successfully!" -ForegroundColor Green

...
$SRC_DIR = Join-Path $ScriptDirAbs "KexPathCch"
$OUT_DIR = Join-Path $ScriptDirAbs "Win32\Release\KexPathCch"

if (-not (Test-Path $OUT_DIR)) { New-Item -ItemType Directory -Path $OUT_DIR | Out-Null }

$src = Join-Path $SRC_DIR "pathcch.c"
$objFile = Join-Path $OUT_DIR "pathcch.obj"
...
$libPath = Join-Path $OUT_DIR "KexPathCch.lib"
$libArgs = @("/NOLOGO", "/LTCG", "/OUT:`"$libPath`"", "`"$objFile`"")

& "lib.exe" @libArgs
if ($LASTEXITCODE -ne 0) { exit 1 }

Write-Host "KexPathCch.lib (x86) built successfully!" -ForegroundColor Green

	...
	IN OUT	PMSG	Message)
{
	...
	Message->wParam = 0;
	Message->lParam = 0;
	if (ShouldCloseTouchInputHandle) {
		// Vista does not have CloseTouchInputHandle
	}
	return TRUE;
}
	...
	IN	ULONG	Flags)
{
	...
	// Vista does not have RegisterTouchWindow
	BOOL Result = TRUE;
...
KXUSERAPI BOOL WINAPI Ext_CloseTouchInputHandle(HTOUCHINPUT hTouchInput) { return TRUE; }
KXUSERAPI BOOL WINAPI Ext_GetTouchInputInfo(HTOUCHINPUT hTouchInput, UINT cInputs, PTOUCHINPUT pInputs, int cbSize) { return FALSE; }
KXUSERAPI BOOL WINAPI Ext_IsTouchWindow(HWND hwnd, PULONG pulFlags) { return FALSE; }

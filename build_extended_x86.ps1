...
$KEXDLL_OUT = Join-Path $ScriptDirAbs "Win32DLLs"
$KEXPATHCCH_OUT = Join-Path $ScriptDirAbs "Win32DLLs"
$KEXSMP_OUT = Join-Path $ScriptDirAbs "Win32DLLs"
$KEXMLS_OUT = Join-Path $ScriptDirAbs "Win32DLLs"
...
foreach ($dllName in $ExtendedDLLs) {
    ...
    $dllOutDir = Join-Path $ScriptDirAbs "Win32\Release\$dllName"
    ...
    if (Test-Path $dllPath) {
        ...
        Copy-Item -Path $dllPath -Destination (Join-Path $ScriptDirAbs "Win32DLLs") -Force
        Copy-Item -Path $libPath -Destination (Join-Path $ScriptDirAbs "Win32DLLs") -Force
    }
}

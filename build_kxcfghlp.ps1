...
Write-Host "VxKex_Vista KxCfgHlp Build with VS2010 (cl.exe)" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
...
$OUT_DIR = Join-Path $ScriptDirAbs "x64\Release\KxCfgHlp"
...
function Invoke-ClCompile {
    ...
    $srcFlag = "`"$SourceFile`""
    $flags = @("/c", "/O1", "/Os", "/Oy", "/GL", "/Gy", "/Gz", "/MD", "/Zi", "/W3", "/TC", "/GS-") + $Defines + $includeFlags + @("/Fd$(Join-Path $OutputDir 'compile.pdb')", $foFlag, $srcFlag)
...
foreach ($srcFile in $Sources) {
    ...
    $defines = @("/D", "WIN32", "/D", "_WIN64", "/D", "NDEBUG", "/D", "_WINDOWS", "/D", "_USRDLL", "/D", "KXCFGHLP_EXPORTS", "/D", "UNICODE", "/D", "_UNICODE")
...
$libPath = Join-Path $OUT_DIR "KxCfgHlp.lib"
...
if (Test-Path $libPath) {
    ...
    Write-Host "  KxCfgHlp.lib: OK ($libSizeKB KB)" -ForegroundColor Green
}

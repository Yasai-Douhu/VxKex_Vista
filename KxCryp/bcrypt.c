	...
	IN	ULONG				Flags)
{
	...
	if (Key->AlgId == ALG_ID_PBKDF2) {
		...
		BCryptOpenAlgorithmProvider(
			&NewAlgorithm,
			AlgorithmType ? AlgorithmType : BCRYPT_SHA1_ALGORITHM,
			NULL,
			BCRYPT_ALG_HANDLE_HMAC_FLAG);
		Status = Ext_BCryptDeriveKeyPBKDF2(
			NewAlgorithm,
			...
			DerivedKeySize,
			0);
		BCryptCloseAlgorithmProvider(NewAlgorithm, 0);
	}
...
//
// PBKDF2 (Password-Based Key Derivation Function 2) の Vista 互換実装。
// Vista の bcrypt.dll には BCryptDeriveKeyPBKDF2 が存在しないため、
// RFC 2898 に従いHMACループを使って完全に自前で実装する。
// HMAC 計算には Vista ネイティブの BCrypt API を使用する。
//

KXCRYPAPI NTSTATUS WINAPI Ext_BCryptDeriveKeyPBKDF2(
	IN		BCRYPT_ALG_HANDLE	Algorithm,
	IN		PUCHAR				Password,
	IN		ULONG				PasswordCb,
	IN		PUCHAR				Salt OPTIONAL,
	IN		ULONG				SaltCb,
	IN		ULONGLONG			IterationCount,
	OUT		PUCHAR				DerivedKey,
	IN		ULONG				DerivedKeyCb,
	IN		ULONG				Flags)
{
	...
	ULONG HashLength;
	ULONG HashLengthCb;
	ULONG HashLengthOutCb;
	ULONG BlockCount;
	ULONG BlockIndex;
	ULONG Offset;
	ULONG CopyLength;
	ULONGLONG Iteration;
	PUCHAR BlockBuffer;		// T_i バッファ（ハッシュ出力）
	PUCHAR UBuffer;			// U_j バッファ（前のHMAC結果）
	PUCHAR TempBuffer;		// 一時バッファ
	UCHAR IndexBytes[4];	// ビッグエンディアンのブロックインデックス
	...
	// 出力バッファはハッシュ長単位のブロック数で分割して生成する
	BlockCount = (DerivedKeyCb + HashLength - 1) / HashLength;

	BlockBuffer = (PUCHAR)RtlAllocateHeap(RtlProcessHeap(), 0, HashLength);
	UBuffer = (PUCHAR)RtlAllocateHeap(RtlProcessHeap(), 0, HashLength);
	if (!BlockBuffer || !UBuffer) {
	...
	for (BlockIndex = 1; BlockIndex <= BlockCount; ++BlockIndex) {
		...
		// ビッグエンディアンのブロックインデックス
		IndexBytes[0] = (UCHAR)((BlockIndex >> 24) & 0xFF);
		IndexBytes[1] = (UCHAR)((BlockIndex >> 16) & 0xFF);
		IndexBytes[2] = (UCHAR)((BlockIndex >> 8) & 0xFF);
		IndexBytes[3] = (UCHAR)(BlockIndex & 0xFF);
		...
		if (Salt && SaltCb > 0) {
			Status = BCryptHashData(HashHandle, Salt, SaltCb, 0);
			if (!NT_SUCCESS(Status)) goto CleanupHashAndExit;
		}
		Status = BCryptHashData(HashHandle, IndexBytes, 4, 0);
		if (!NT_SUCCESS(Status)) goto CleanupHashAndExit;

		Status = BCryptFinishHash(HashHandle, UBuffer, HashLength, 0);
		if (!NT_SUCCESS(Status)) goto CleanupHashAndExit;
		...
		CopyMemory(BlockBuffer, UBuffer, HashLength);
		...
		for (Iteration = 1; Iteration < IterationCount; ++Iteration) {
			...
			Status = BCryptHashData(HashHandle, UBuffer, HashLength, 0);
			if (!NT_SUCCESS(Status)) goto CleanupHashAndExit;
			...
			// XOR で累積する
			{
				...
				for (i = 0; i < HashLength; ++i) {
					BlockBuffer[i] ^= UBuffer[i];
				}
			}
		}

		// 出力バッファへのコピー（最終ブロックは残りバイト数のみ）
		CopyLength = min(HashLength, DerivedKeyCb - Offset);
		CopyMemory(DerivedKey + Offset, BlockBuffer, CopyLength);
		Offset += CopyLength;
	}
...
CleanupAndExit:
	if (BlockBuffer) RtlFreeHeap(RtlProcessHeap(), 0, BlockBuffer);
	if (UBuffer) RtlFreeHeap(RtlProcessHeap(), 0, UBuffer);

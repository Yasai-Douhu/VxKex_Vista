...
Write-Host "VxKex_Vista KexGui Build with VS2010 (cl.exe)" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
...
$SRC_DIR = Join-Path $ScriptDirAbs "KexGui"
$OUT_DIR = Join-Path $ScriptDirAbs "Win32\Release\KexGui"
...
Write-Host "[1/2] Compiling KexGui source files..." -ForegroundColor Yellow

$Sources = @("ctlsx.c", "dpi.c", "fldrpkr.c", "kexgui.c", "locale.c", "mls.c", "msgbox.c", "shprops.c", "window.c", "wndx.c")
$ObjFiles = @()
$allOk = $true
...
foreach ($srcFile in $Sources) {
    ...
    $defines = @("/D", "WIN32", "/D", "NDEBUG", "/D", "_WINDOWS", "/D", "_USRDLL", "/D", "KEXGUI_EXPORTS", "/D", "UNICODE", "/D", "_UNICODE")
...
$libPath = Join-Path $OUT_DIR "KexGui.lib"

Write-Host "" -ForegroundColor Yellow
Write-Host "[2/2] Archiving KexGui.lib..." -ForegroundColor Yellow

$libArgs = @("/NOLOGO", "/LTCG", "/OUT:`"$libPath`"")
...
if (Test-Path $libPath) {
    ...
    Write-Host "  KexGui.lib: OK ($libSizeKB KB)" -ForegroundColor Green
}
